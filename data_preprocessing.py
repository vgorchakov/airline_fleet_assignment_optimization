...
class DataPreprocess():
    ...
    def preprocess_data(self):
        ...
        flights_data['Scheduled.Departure'] = pd.to_datetime(flights_data['Scheduled.Departure'])
        flights_data['Scheduled.Arrival'] = pd.to_datetime(flights_data['Scheduled.Arrival'])
        flights_data['Departure_Date'] = flights_data['Scheduled.Departure'].dt.date

        # take one day with maximum number of flights
        day_max_number_flights = flights_data.groupby('Departure_Date').size().idxmax()
        one_day_flights_df = flights_data[flights_data['Departure_Date'] == day_max_number_flights]

        # preprocess dataframe of one day flights
